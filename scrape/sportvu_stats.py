...
def add_game_id_to_game_log_for_player(daily_data, date, storage):
    ...
    for row in daily_data:
        ...
        for game in games:
            game_id = game[0]
            boxscore = storage.query("select * from traditional_boxscores where PLAYER_ID = " + player_id + " and GAME_ID = " + game_id)
            if len(boxscore) > 0:
                row["GAME_ID"] = game_id
...
def add_game_id_to_game_log_for_team(daily_data, date, storage):
    games = storage.query('select GAME_ID from game_summary where GAME_DATE_EST = "' + date + 'T00:00:00"')
    to_return = []
    for row in daily_data:
        ...
        for game in games:
            game_id = game[0]
            boxscore = storage.query("select * from traditional_boxscores_team where TEAM_ID = " + team_id + " and GAME_ID = " + game_id)
            if len(boxscore) > 0:
                row["GAME_ID"] = game_id
                to_return.append(row)
                break
